...
def ggpl_house_builder():
	...
	with open("Lines_Files/porte_new.lines", "rb") as csvfile:
		csvReader = csv.reader(csvfile,delimiter=",")
		listDoorsHoles = []
		cuboid = []
		initHoles = True
		...
		for row in csvReader:
			...
			if cont <=2:
				...
				elif round(float(row[1]),1) != round(float(row[3]),1) and round(float(row[0]),1) != round(float(row[2]),1):
					yDiff = abs(float(row[3]) - float(row[1]))
					xDiff = abs(float(row[2]) - float(row[0]))
					slantingDoor = True
			...
			if cont == 4:
				...
				if slantingDoor == True:
					door = R([1,2])(PI/4)(door)
					slantingDoor = False
